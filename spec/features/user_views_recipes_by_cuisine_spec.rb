...
feature 'User views recipes by cuisine' do
  scenario 'successfully' do
    ...
    expect(page).to have_css('h1', text: cuisine.name.downcase)
  ...
  scenario 'and view only the recipes from current cuisine' do
    ...
    recipe_a = create(:recipe, cuisine: cuisine_1)
    ...
    expect(page).to have_css('h1', text: cuisine_2.name)
    expect(page).to have_content recipe_b.title
    expect(page).not_to have_content recipe_a.title
    expect(page).not_to have_content cuisine_1.name
  end

  scenario 'view a friendly empty message' do
    cuisine = Cuisine.create(name: 'sobremesas')

    visit root_path
    click_on cuisine.name

    expect(page).to have_content 'Nenhuma receita adicionada.'
  end
